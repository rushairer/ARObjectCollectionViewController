...
- (void)showWithRootController:(UINavigationController *)rootViewController
{
    if (self.objectCollection)
    {
        ...
        {
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSStringFromClass([self.objectCollection class])
                                                                           message:[self.objectCollection description]
...
- (NSString *)detailTextForIndexPath:(NSIndexPath *)indexPath
{
    if ([self.objectCollection isKindOfClass:[NSDictionary class]])
    {
        ...
        return [[self valueForIndexPath:indexPath] description];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        if ([value isKindOfClass:[NSString class]] && [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:value]])
        {
            ...
            {
                [self showStringAlertView:[value description]];
            }
        }
...
-(void)showStringAlertView:(NSString *)string
{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil
                                                                   message:string
                                                            preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *copy = [UIAlertAction actionWithTitle:NSLocalizedString(@"Copy", nil)
